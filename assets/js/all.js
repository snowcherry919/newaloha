      ...
      $.ajax({
        ...
        success: function success(data) {
          ...
          if (data.message == "此帳號已被使用") {
            ...
            $('#erro_signupEmail').css("color", "red"); //帳號如果存在，讓btn無法按

            $('#signupBtn').attr('disabled', true);
            $flag_checkuni = false;
          } else {
            $('#erro_signupEmail').html("此帳號可以使用");
            $('#erro_signupEmail').css("color", "green"); //帳號如果沒有註冊過，讓btn可以按
...
var showLocation = document.getElementById('serchBar'); //讓使用者選到的地區秀在btn上

function checkLocation(e) {
  e.preventDefault(); //取消a連結的預設動作

  var selectLocation = e.target.textContent;
  showLocation.innerHTML = "<span class=\"mb-0 text-center\"><i class=\"fas fa-map-marker-alt mr-3 text-dark\"></i>".concat(selectLocation, "<span>");
}

locationBtn.addEventListener('click', checkLocation, false); // function changeRoomCount(e){
//     console.log(e.target);
// }
// roomCount.addEventListener('click',changeRoomCount);

var guests = document.getElementById('dropdownGuests'); //操控下拉選單開啟與關閉的時機
...
guests.addEventListener('click', showData); // 人數按鈕
// adult減號按鈕
...
function del(id) {
  var num = parseInt(id.textContent) - 1; //如果人數少於0，將它設為0，不要有負數
...
function cDel(id) {
  var num = parseInt(id.textContent) - 1;
...
function getTotal() {
  //取得成人的人數
  var adultSum = parseInt(adult.textContent); //取得小孩的人數

  var childSum = parseInt(child.textContent); //取得房間數量

  var roomSum = parseInt(room.textContent); //將人數加總

  var totalSum = adultSum + childSum; //寫回button內容中

  guests.innerHTML = "<span class=\"text-third\"><i class=\"fas fa-user-alt mr-3 text-dark\"></i>".concat(totalSum, " guests / ").concat(roomSum, " room </span>");
} //當按下-按鈕時
...
function getSerchData(e) {
  ...
  var saveLocation = showLocation.innerText; //存取查詢時間

  var saveReserveDay = reserveDay.innerText; //存取人數房間
